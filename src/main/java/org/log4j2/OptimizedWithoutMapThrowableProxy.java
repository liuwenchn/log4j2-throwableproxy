 ...
 * </p>
 */
public class OptimizedWithoutMapThrowableProxy implements Serializable {
    ...
    private static final OptimizedWithoutMapThrowableProxy[] EMPTY_THROWABLE_PROXY_ARRAY = new OptimizedWithoutMapThrowableProxy[0];
    ...
    private final OptimizedWithoutMapThrowableProxy causeProxy;
    ...
    private final OptimizedWithoutMapThrowableProxy[] suppressedProxies;
    ...
    private OptimizedWithoutMapThrowableProxy() {
        this.throwable = null;
        this.name = null;
     ...
     */
    public OptimizedWithoutMapThrowableProxy(final Throwable throwable) {
        this(throwable, null);
    }
     ...
     */
    private OptimizedWithoutMapThrowableProxy(final Throwable throwable, final Set<Throwable> visited) {
        this.throwable = throwable;
        this.name = throwable.getClass().getName();
        this.message = throwable.getMessage();
        this.localizedMessage = throwable.getLocalizedMessage();
        final Stack<Class<?>> stack = StackLocatorUtil.getCurrentStackTrace();
        final  Map<String, OptimizedWithoutMapThrowableProxy.CacheEntry> map = new HashMap<>();
        this.extendedStackTrace = this.toExtendedStackTrace(stack, map, null, throwable.getStackTrace());
        final Throwable throwableCause = throwable.getCause();
        final Set<Throwable> causeVisited = new HashSet<>(1);
        this.causeProxy = throwableCause == null ? null : new OptimizedWithoutMapThrowableProxy(throwable, stack, map, throwableCause,
     ...
     */
    private OptimizedWithoutMapThrowableProxy(final Throwable parent, final Stack<Class<?>> stack, final Map<String, CacheEntry> map,
        ...
        this.causeProxy = causeCause == null || causeVisited.contains(causeCause) ? null : new OptimizedWithoutMapThrowableProxy(parent,
    ...
    public boolean equals(final Object obj) {
        ...
        final OptimizedWithoutMapThrowableProxy other = (OptimizedWithoutMapThrowableProxy) obj;
    ...
    private void formatCause(final StringBuilder sb, final String prefix, final OptimizedWithoutMapThrowableProxy cause,
    ...
    private void formatThrowableProxy(final StringBuilder sb, final String prefix, final String causeLabel,
                                      final OptimizedWithoutMapThrowableProxy throwableProxy, final List<String> ignorePackages,
    ...
    private void formatSuppressed(final StringBuilder sb, final String prefix, final OptimizedWithoutMapThrowableProxy[] suppressedProxies,
                                  final List<String> ignorePackages, final TextRenderer textRenderer, final String suffix) {
        if (suppressedProxies == null) {
            return;
        }
        for (final OptimizedWithoutMapThrowableProxy suppressedProxy : suppressedProxies) {
    ...
    private void formatEntry(final ExtendedStackTraceElement extStackTraceElement, final StringBuilder sb,
        ...
        renderOn(extStackTraceElement, sb, textRenderer);
        renderSuffix(suffix, sb, textRenderer);
        textRenderer.render(EOL_STR, sb, "Text");
    }

    void renderOn(ExtendedStackTraceElement ext, final StringBuilder output, final TextRenderer textRenderer) {
        render(ext, ext.getStackTraceElement(), output, textRenderer);
        textRenderer.render(" ", output, "Text");
        ext.getExtraClassInfo().renderOn(output, textRenderer);
    }

    private void render(ExtendedStackTraceElement ext, final StackTraceElement stElement, final StringBuilder output, final TextRenderer textRenderer) {
        final String fileName = stElement.getFileName();
        final int lineNumber = stElement.getLineNumber();
        textRenderer.render(ext. getClassName(), output, "StackTraceElement.ClassName");
     ...
     * @param suffix
     */
    public void formatWrapper(final StringBuilder sb, final OptimizedWithoutMapThrowableProxy cause, final String suffix) {
    ...
    public void formatWrapper(final StringBuilder sb, final OptimizedWithoutMapThrowableProxy cause, final List<String> ignorePackages, final String suffix) {
     ...
     */
    @SuppressWarnings("ThrowableResultOfMethodCallIgnored")
    public void formatWrapper(final StringBuilder sb, final OptimizedWithoutMapThrowableProxy cause, final List<String> ignorePackages,
                              final TextRenderer textRenderer, final String suffix) {
    ...
    public OptimizedWithoutMapThrowableProxy getCauseProxy() {
     ...
     */
    public OptimizedWithoutMapThrowableProxy[] getSuppressedProxies() {
    ...
    public String getSuppressedStackTrace(final String suffix) {
        final OptimizedWithoutMapThrowableProxy[] suppressed = this.getSuppressedProxies();
        if (suppressed == null || suppressed.length == 0) {
            return Strings.EMPTY;
        }
        final StringBuilder sb = new StringBuilder("Suppressed Stack Trace Elements:").append(EOL);
        for (final OptimizedWithoutMapThrowableProxy proxy : suppressed) {
    ...
    private OptimizedWithoutMapThrowableProxy[] toSuppressedProxies(final Throwable thrown, Set<Throwable> suppressedVisited) {
        try {
            final Throwable[] suppressed = thrown.getSuppressed();
            if (suppressed == null) {
                return EMPTY_THROWABLE_PROXY_ARRAY;
            }
            final List<OptimizedWithoutMapThrowableProxy> proxies = new ArrayList<>(suppressed.length);
            ...
            for (int i = 0; i < suppressed.length; i++) {
                ...
                if (!suppressedVisited.contains(candidate)) {
                    ...
                    proxies.add(new OptimizedWithoutMapThrowableProxy(candidate, suppressedVisited));
                }
            }
            return proxies.toArray(new OptimizedWithoutMapThrowableProxy[proxies.size()]);
